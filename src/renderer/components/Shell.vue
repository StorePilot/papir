      ...
      <el-row style="max-height: calc(50vh - 38px); overflow-y: scroll;">
        <el-col :sm="24" :lg="12">
          <el-card style="margin: 20px 10px 10px;" class="box-card">
            ...
            <pre style="max-width: 100%; overflow-x: scroll">{{request}}</pre>
          </el-card>
        </el-col>
        <el-col :sm="24" :lg="12">
          <el-card style="margin: 20px 10px 0 10px;" class="box-card">
            ...
            <pre style="max-width: 100%; overflow-x: scroll">{{response}}</pre>
          </el-card>
        </el-col>
      </el-row>
    </el-col>
          ...
          switch (method) {
            case 'FETCH':
              endpoint.fetch(apiSlug, args, replace, perform).then(config => {
                this.request = config
                if (typeof this.request.cancelToken !== 'undefined') {
                  delete this.request.cancelToken
                }
              })
              break
            case 'SAVE':
